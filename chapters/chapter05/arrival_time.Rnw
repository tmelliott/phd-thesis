...
Another way to make predictions that takes no account of the real-time state of the vehicle, trip, or network, is to use historical data. We collected two weeks of data and recorded arrival delays by stop and trip to obtain a distribution.


The historical data-based predictions are, therefore, that the arrival delay at stop $j$ along trip $\Tripr$ has mean $\hat\alpha_{rj}$ and variance $\hat\zeta_{rj}^2$, so if the scheduled arrival time is $S_{rj}$, then the predicted arrival time is
\begin{equation}
\label{eq:arrival_pred_historical}
\Tarr_{\Tripr j} \sim \Normal{S_{\Tripr j} + \hat\alpha_{\Tripr j}}{\hat\zeta_{\Tripr j}^2}.
\end{equation}
...
\end{equation}
Note, however, that if no observations for a given trip have been received, the default delay is $\bar\delta_r = 0$, regardless of whether or not the trip is running.
