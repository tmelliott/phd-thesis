...
<<normal_approx,cache=TRUE,echo=FALSE,fig.width=8,fig.height=4,fig.cap="Normal approximation for a series of stops ahead. The true distribution is shown by the histogram with the underlying components (dashed curves). The vertical lines represent the quantiles computed using the sampled data, the normal mixture (using the optimisation formula), or a single normal approximation.",out.width="\\textwidth",fig.align="center">>=
