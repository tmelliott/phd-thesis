...
\item Demonstration that the real-time application (implemented within the \pkg{transitr} package) runs well within the target timeframe (10~seconds or less per iteration).
\end{itemize}
