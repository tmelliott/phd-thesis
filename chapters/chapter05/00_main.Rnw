...
traffic lights, etc.
It is therefore reasonable to return to the particle filter
...
so we have developed a Kalman filter to smooth the \glspl{eta} over time.
...
\item update the travel time state vector using a Kalman filter update
...
\Sexpr{knit_child('particle_etas.Rnw')}
\Sexpr{knit_child('trip_eta_distributions.Rnw')}
