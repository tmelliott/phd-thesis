...
In order to convert the travel time state
into arrival time estimates,
we need to describe the temporal and spatial relation
between road segments,
and how these are combined to estimate arrival time.
