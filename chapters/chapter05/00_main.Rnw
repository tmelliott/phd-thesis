...
\end{enumerate}
Knowing these allows us to express the arrival time at stop $j$,
of a vehicle last observed at time $\Vtime_k$, as
\begin{equation}
\label{eq:trip_eta}
\Tarr_j = \Vtime_k + \sum_{m=1}^{j-1} \Linkt_m + \Tdwell_m
\end{equation}
