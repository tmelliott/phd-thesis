...
For dwell times, 
we do not yet have any real-time model,
...
\dot D_m \sim \mathcal{N}(\tau_m, \omega_m^2) T(0,),\quad
...
<<eta_dwell_times, echo=FALSE, fig.height=5,fig.cap="Dwell time at bus stop">>=
gamma <- 10
tau <- 20
omega <- 10
...
points(0, pi, pch = 19)
axis(2, at = c(0, pi), label = c(0, expression(1-pi)), las = 2)
axis(1, at = c(0, gamma, tau), label = c(0, expression(gamma), expression(tau)))
@
