...
tbl <- knitr::kable(
    ...
    format = "latex",
    booktabs = TRUE,
    caption = "Predictive model comparison of RMSE and MAE, in seconds, MAPE (\\%), PPI coverage, and mean PPI width, in minutes, with (0.025, 0.975) quantiles.",
    escape = FALSE
)
tbl <- gsub("& SKIP", "", tbl)
...
<<model_results_rmse_time,warning=FALSE,echo=FALSE,cache=TRUE,dependson=c("model_results_load","model_colours"),cache.lazy=FALSE,fig.width=8,fig.height=5,out.width="\\textwidth",fig.cap="Model comparative statistics as a function of time until arrival. Note the log-scale for RMSE, MAE, and MAPE.">>=
# tidyverse is loaded!
res_min <- eta_results %>%
    mutate(
...
<<model_results_rmse_stopn,warning=FALSE,echo=FALSE,cache=TRUE,dependson=c("model_results_load","model_colours"),cache.lazy=FALSE,fig.width=8,fig.height=5,out.width="\\textwidth",fig.cap="Model comparative statistics as a function of stop sequence.">>=
# library(patchwork)
# tidyverse is loaded!
...
<<model_results_rmse_timeofday,warning=FALSE,echo=FALSE,cache=TRUE,dependson=c("model_results_load","model_colours"),cache.lazy=FALSE,fig.width=8,fig.height=5,out.width="\\textwidth",fig.cap="Model comparative statistics as a function of time of day.">>=
...
<<model_results_pr_time,echo=FALSE,message=FALSE,warning=FALSE,cache=TRUE,dependson=c("model_results_load","model_results_pr_miss"),cache.lazy=FALSE,fig.width=8,fig.height=4.5,out.width="\\textwidth",fig.cap="Summary values by time until arrival.">>=
...
plotC <- plot_base +
    ...
    geom_ribbon(
        aes(
            ymin = pmin(20, pmin(min, pf_wait_lower / 60)),
            ymax = pmin(20, pmin(min, pf_wait_upper / 60)),
            fill = "pf"
        ),
        alpha = 0.2
    ) +
    geom_ribbon(
        aes(
            ymin = pmin(20, pmin(min, normal_wait_lower / 60)),
            ymax = pmin(20, pmin(min, normal_wait_upper / 60)),
            fill = "normal"
        ),
        alpha = 0.2
    ) +
    geom_ribbon(
        aes(
            ymin = pmin(20, pmin(min, hist_wait_lower / 60)),
            ymax = pmin(20, pmin(min, hist_wait_upper / 60)),
            fill = "historical"
        ),
        alpha = 0.2
    ) +
...
<<model_results_pr_stop,echo=FALSE,message=FALSE,warning=FALSE,cache=TRUE,dependson=c("model_results_load","model_results_pr_miss"),cache.lazy=FALSE,fig.width=8,fig.height=4.5,out.width="\\textwidth",fig.cap="Summary values by stop sequence.">>=
...
plotC <- plot_base +
    geom_ribbon(
        aes(
            ymin = pmin(20, pf_wait_lower / 60),
            ymax = pmin(20, pf_wait_upper / 60),
            fill = "pf"
        ),
        alpha = 0.2
    ) +
    geom_ribbon(
        aes(
            ymin = pmin(20, normal_wait_lower / 60),
            ymax = pmin(20, normal_wait_upper / 60),
            fill = "normal"
        ),
        alpha = 0.2
    ) +
    geom_ribbon(
        aes(
            ymin = pmin(20, hist_wait_lower / 60),
            ymax = pmin(20, hist_wait_upper / 60),
            fill = "historical"
        ),
        alpha = 0.2
    ) +
