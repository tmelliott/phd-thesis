...
# totals:
if (is.null(timings$ntripupdates))
    timings$ntripupdates <- timings$nvehicles * 1.1
total_time <- timings %>%
    group_by(timestamp) %>%
    summarize(
        wall = sum(wall),
...
p1 <- p0 +
    geom_path(aes(y = nvehicles, colour = "Vehicle Positions"),
        data = total_time
    ) +
    geom_path(aes(y = ntripupdates, colour = "Trip Updates"),
        data = total_time
    ) +
