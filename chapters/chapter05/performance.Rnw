...
<<prediction_timing_time,warning=FALSE,echo=FALSE,cache=TRUE,dependson="prediction_timing",fig.width=8*.8,fig.height=3,out.width=".8\\textwidth",fig.align="center",fig.cap="Timing results over time for various stages of the program: (L) Load data, (O) Update vehicles information, (V) Vehicle state update, (N) Network state update, (P) Predict ETAs, (W) Write ETAs to protobuf feed, (T) Total iteration time.">>=
# totals:
total_time <- timings %>%
    group_by(timestamp) %>%
...
timings %>%
    ...
    ggplot(aes(timestamp)) +
        ...
        xlab("Time") +
        scale_y_continuous(
            "Iteration timing (milliseconds, wall clock)",
            trans = "log10"
        )
@
