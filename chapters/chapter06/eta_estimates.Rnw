...
<<eta_calc_quantile,echo=FALSE,fig.width=8*.6,fig.height=2.5,out.width=".6\\textwidth",fig.cap="Quantile estimation from a CDF. ETA values with quantiles below the desired threshold are coloured red; the maximum of these is the quantile value.",fig.align="center">>=
...
ggplot(d, aes(t, q)) +
  ...
  geom_segment(
    aes(x = t, xend = t, y = 0, yend = q),
    data = d[max(which(d$q < 0.5)), ],
    lty = 2
  )
