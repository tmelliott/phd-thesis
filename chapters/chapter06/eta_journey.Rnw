...
ggmap(map) +
  ...
  labs(colour = "Journey leg")

@



<<eta_journey_transfer_prob,echo=FALSE,cache=TRUE,dependson="-1">>=

TSTART <- as.POSIXct(paste(DATE, "7:00:00"))
con <- dbConnect(SQLite(), "data/eta_quantiles.sqlite")
trip_etas <- con %>% tbl("etas") %>%
  filter(
...
ggplot(trip_etas, aes(timestamp + eta * 60, quantile)) +
  geom_path(aes(group = trip_id, colour = transfer))
...
trip_110_depart <- tibble(
  time = as.POSIXct(paste("2019-08-29", c("7:05:00", "7:35:00"))) - 30*60
)


TSTART <- as.POSIXct("2019-08-29 6:25:00")
...
royal110 <- trip_110 %>%
  filter(stop_sequence == 6 & trip_start_time == "06:35") %>%
  summarize(
    arrival = as.POSIXct(min(actual_arrival), origin = "1970-01-01")
  ) %>%
  mutate(
...
cbd110 <- trip_110 %>%
  filter(stop_sequence == 29 & trip_start_time == "06:35") %>%
  summarize(
    arrival = as.POSIXct(min(actual_arrival), origin = "1970-01-01")
  ) %>%
  mutate(
