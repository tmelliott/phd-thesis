...
In this scenario, a passenger lives within walking distance of two major roads, along which various routes travel into the central city, as displayed in \cref{fig:eta_journey_arrival_prep}. The passenger needs to decide which route option to take before leaving home, and let's say they have an appointment in town at \Sexpr{format(TEND, "%Hh%M")}. At \Sexpr{format(TSTART, "%Hh%M")}, they consult the real-time app on their phone before deciding whether they will walk to route option A or B. Factors that could influence their decision include:
\begin{itemize}
...
options(knitr.kable.NA = "")
knitr::kable(
  cbind("GTS Prediction" = c("No", "Yes"), gtfs_catch_pc, NA, gtfs_arrive_pc),
  row.names = FALSE,
  format = "latex",
  booktabs = TRUE,
...
The results in \cref{fig:eta_journey_arrival,fig:eta_journey_arriveby} and \cref{tab:eta_journey_results} are based on \emph{one single forecast} made at \Sexpr{format(TSTART, "%kh%M")}. To evaluate the overall performance of our prediction method, we repeated the process described above in 5~minute intervals over the off-peak period from \Sexpr{format(min(tstart), "%kh%M")} to \Sexpr{format(max(tstart), "%kh%M")}. For the appointment time, we used 30~minute intervals allowing for 15--45 minutes for the journey: leaving between 9h15 and 9h44 had a targetted arrival time of 8h00; 9h45--10h14 targetted 8h30; and so on. The probabilities of catching the bus and arriving on time were calculated for each case. In \cref{fig:eta_journey_results_avg}, we have graphed the distribution of predicted probabilities for each of the two outcomes, for both catching the bus and arriving on time. Only a small number of missed buses had predictions over 50\%, though a larger number of potential buses had low capture probabilities. As for the arrival outcome, successfully arriving on time was usually predicted well, with a small number of low predictions; however, the maximum probability assigned to a failed on-time arrival was 75\%.



For comparison, \cref{tab:eta_journey_results_avg} presents a two-way contingency table for the binary schedule-delay predictions, with the predicted outcomes in rows. In \Sexpr{round(100 * gtfs_catch_pc[2,2])}\% of the cases where the bus was predicted to arrive after the passenger was the schedule-delay method correct, and \Sexpr{round(100 * gtfs_catch_pc[1,1])}\% for when it predicted the passenger would miss the bus. For arrival on time, the bus correctly predicted `Yes' \Sexpr{round(100 * gtfs_arrive_pc[2,2])}\% of the time, but only correctly predicted `No' in \Sexpr{round(100 * gtfs_arrive_pc[1,1])}\% of cases. Overall, the schedule-delay method correctly predicted the bus capture in \Sexpr{round(100 * sum(diag(gtfs_catch_tab)) / sum(gtfs_catch_tab))}\% of cases, and correctly predicted on-time arrival in \Sexpr{round(100 * sum(diag(gtfs_arrive_tab)) / sum(gtfs_arrive_tab))}\% of cases.
...
all_stop_arrivals <- trip_etas %>%
  ...
  mutate(
    which = factor(which, levels = c("Start", "Transfer", "End"))
  )

# now filter out irrelevant trips...
trips_at_start <- all_stop_arrivals %>%
  filter(which == "Start" & transfer == 1) %>%
  pull(trip_id) %>% unique
...
As before, we proceed to repeat the procedure for multiple times (this time between \Sexpr{format(min(tstart), "%kh%M")} and \Sexpr{format(max(tstart), "%kh%M")}), and compute the probabilities of available transfers and whether or not they connected. \Cref{fig:eta_journey_transfer_many} shows the predicted transfer probabilities grouped by outcome, and coloured by the schedule-delay prediction. This is complimented with contingency \cref{tab:eta_journey_transfer_many} for the schedule-delay method and our own, respectively. For the schedule-delay method, in \Sexpr{round(100*gtfs_pc[2,1])}\% of cases a predicted successfull connection failed and showed a \Sexpr{round(100*gtfs_pc[1,2])}\%, versus only \Sexpr{round(100*pf_pc[2,1])}\% using the \pf{} with a decision rule of $\Pcatch > 0.5$. The false negative rate was similar for both methods, although slightly higher for our own (\Sexpr{round(100*pf_pc[1,2])}\% versus \Sexpr{round(100*gtfs_pc[1,2])}\% for schedule-delay). However, depending on the passenger's requirements, it is possible to reduce the false positive rate under the \pf{} by increasing the threshold. \Cref{tab:eta_journey_transfer_many} also shows the results using 0.8 and 0.95 as the decision rule, which result in a \Sexpr{round(100*pf_pc2[2,1])}\% and \Sexpr{round(100*pf_pc3[2,1])}\% false positive rate, respectively, though these result in quite a significant increase in the false negative rate. Again, by providing probabilities the passenger has the option to decide what level of risk they are willing to take.
