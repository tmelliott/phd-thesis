...
gtfs_catch_tab <- table(
  gtfs = ifelse(mdata$gtfs_catch, "Yes", "No"),
...
gtfs_arrive_tab <- table(
  gtfs = ifelse(mdata$gtfs_arrive, "Yes", "No"),
...
options(knitr.kable.NA = "")
knitr::kable(cbind(gtfs_catch_pc, NA, gtfs_arrive_pc),
  format = "latex",
  booktabs = TRUE,
  caption = "GTFS prediction outcomes for arriving at stop before bus.",
  escape = FALSE,
  linesep = ""
) %>%
...
route_info <- con %>% tbl("routes") %>%
  ...
  collect() %>%
  mutate(
    transfer = ifelse(route_short_name %in% c("27H", "27W"), "A", "B")
  )
trip_info <- con %>% tbl("trips") %>%
  filter(route_id %in% !!route_info$route_id) %>%
  ...
  group_modify(~{
    .x %>% mutate(
      trip_start_time = paste0(
        .y$transfer,
        as.integer(as.factor(trip_start_time))
      )
    )
  }) %>%
  ungroup()
...
\begin{split}
\Pr{A < B} &= \sum_{x=1}^{\infty} \Pr{A < B\,|\,B = x} \\
  &= \sum_{x=1}^\infty
    \Pr{A < x} \Pr{B=x} \\
  &= \sum_{x=1}^\infty
    \Pr{A < x} \left[
...
\end{split}
\end{equation}
which we can easily compute given the \glspl{cdf} of the arrival time of each trip obtained from \cref{eq:pf_cdf_arrivaltime}. Of course, this assumes independence, which probably isn't true but we don't have any information on it.
...
transfer_arrivals <- stop_arrivals %>%
  filter(which == "Transfer" & transfer == "A") %>%
  select(trip_id, trip_start_time, timestamp, eta, quantile,
    gtfs_eta, actual_arrival)
transfer_departures <- stop_arrivals %>%
  filter(which == "Transfer" & transfer == "B") %>%
  select(trip_id, trip_start_time, timestamp, eta, quantile,
    gtfs_eta, actual_arrival)
...
tab <- tibble(
  ...
  p_catch = round(f(A, B, 1, transfer_arrivals, transfer_departures), 2),
  gtfs_eta = f(A, B, 2, transfer_arrivals, transfer_departures),
  did_catch = f(A, B, 3, transfer_arrivals, transfer_departures)
) %>%
  rename(
    "$\\mathbb{P}_\\text{catch}" = p_catch,
    "GTFS" = gtfs_eta,
    "Outcome" = did_catch
  )
...
Now just to replicate with lots of different times to see how often our choice is right vs gtfs.

<<eta_journey_transfer_many,echo=FALSE,cache=TRUE,dependson=-1,message=FALSE,fig.width=8,fig.height=3,out.width="\\textwidth",fig.cap="Results of performing the same transfer journey planning prediction with different starting times (from 6am to 8pm). Observations are whether or not the first bus would arrive at the transfer stop before the second bus, jittered to better see each observation. The smoothed curve represents the mean, and indicates that on average our method is good at determining if the bus can be caught or not. Additionally, predictive probabilities of 0 and 1 have been removed since, after checking, none or these were invalid and only served to distort the figure.",fig.align="center">>=


# 5-minute intervals from 6am--6pm:
tstart <- seq(
  as.POSIXct("2019-08-29 6:00:00"),
  as.POSIXct("2019-08-29 20:00:00"),
  by = 5 * 60
)
...
knitr::kable(pf_pc,
  format = "latex",
  booktabs = TRUE,
  caption = "PF prediction outcomes using rule: P > 0.5",
  escape = FALSE,
  linesep = ""
)
