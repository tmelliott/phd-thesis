...
f <- Vectorize(
  function(a, b, what = 1) {
    ta <- transfer_arrivals %>% filter(trip_start_time == a)
    tb <- transfer_departures %>% filter(trip_start_time == b)

    if (what == 1) {
      etas <- 1:max(ta$eta, tb$eta)
      ea <- ifelse(etas < max(ta$eta), 0, 1)
      eb <- ifelse(etas < max(tb$eta), 1, 0)

      ea[ta$eta] <- ta$quantile
      eb[tb$eta] <- 1 - tb$quantile
    ...
    if (what == 2) {
      ...
      ga <- ta$timestamp[1] + ta$eta[1] * 60
      gb <- tb$timestamp[1] + tb$eta[1] * 60
