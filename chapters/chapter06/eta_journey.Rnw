...
tab <- trip_eta_results_comb %>%
  select(option, route_short_name, trip_start_time, p_catch, journey_length,
    p_arrive, gtfs_catch, gtfs_arrive, did_catch, did_arrive) %>%
  mutate(
...
options(knitr.kable.NA = "")
knitr::kable(
  cbind(gtfs_catch_pc, NA, gtfs_arrive_pc),
  format = "latex",
  booktabs = TRUE,
...
<<eta_journey_transfer_many2,echo=FALSE,cache=TRUE,dependson=-1,message=FALSE>>=
