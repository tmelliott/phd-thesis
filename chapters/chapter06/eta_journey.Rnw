...
trip_111 <- trip_etas %>%
  ...
  filter(stop_sequence == !!rseq_111$stop_max) %>%
  collect() %>%
  mutate(timestamp = as.POSIXct(timestamp, origin = "1970-01-01")) %>%
  left_join(trip_info, by = "trip_id")
...
trip_110_depart <- tibble(
  time = as.POSIXct(paste("2019-08-29", c("6:35:00", "7:05:00", "7:35:00")))
)


TSTART <- as.POSIXct("2019-08-29 7:00:00")
trip_125X_depart <- trip_125X %>% filter(timestamp <= TSTART) %>%
  group_by(trip_id) %>%
  group_modify(~{
    .x %>% filter(timestamp == max(timestamp))
  }) %>%
  arrange(trip_start_time, eta)

trip_111 %>% filter(timestamp <= TSTART) %>%
  group_by(trip_id) %>%
  group_modify(~{
    .x %>% filter(timestamp == max(timestamp))
  }) %>%
  ...
  ggplot(aes(timestamp + eta * 60, quantile, group = trip_start_time)) +
    ...
    geom_path(aes(colour = "125X"), data = trip_125X_depart)
