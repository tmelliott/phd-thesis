...
<<eta_overall_results,echo=FALSE,cache=TRUE,dependson="eta_load",fig.width=8,fig.height=5,out.width="\\textwidth",fig.align="center",fig.cap="ETA stuff">>=
...
pf_mae90 <- with(q90, mean(abs(pf - actual)))
pf_mae75 <- with(q75, mean(abs(pf - actual)))
pf_mae60 <- with(q60, mean(abs(pf - actual)))
pf_mae50 <- with(q50, mean(abs(pf - actual)))
pf_mae40 <- with(q40, mean(abs(pf - actual)))
pf_mae25 <- with(q25, mean(abs(pf - actual)))
pf_mae05 <- with(q05, mean(abs(pf - actual)))
gtfs_mae <- with(q50, mean(abs(gtfs - actual)))
mae <- c(pf_mae90, pf_mae75, pf_mae60, pf_mae50,
  pf_mae40, pf_mae25, pf_mae05, gtfs_mae)

pf_mape90 <- with(q90 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape75 <- with(q75 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape60 <- with(q60 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape50 <- with(q50 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape40 <- with(q40 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape25 <- with(q25 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
pf_mape05 <- with(q05 %>% filter(actual > 0),
  mean(abs((pf - actual) / actual), na.rm = TRUE))
gtfs_mape <- with(q50 %>% filter(actual > 0),
  mean(abs((gtfs - actual) / actual), na.rm = TRUE))
mape <- c(pf_mape90, pf_mape75, pf_mape60, pf_mape50,
  pf_mape40, pf_mape25, pf_mape05, gtfs_mape)
...
d <- tibble(
  m = c("PF90", "PF75", "PF60", "PF50",
    "PF40", "PF25", "PF05", "GTFS"),
  rmse = rmse, mae = mae, mape = 100 * mape,
  pcatch = pcatch, ewait = ewait
)

p1 <- ggplot(d, aes(rmse, m)) +
  geom_point() + geom_segment(aes(xend = 0, yend = m)) +
  scale_x_continuous("RMSE (minutes)")
p2 <- ggplot(d, aes(mae, m)) +
  geom_point() + geom_segment(aes(xend = 0, yend = m)) +
  scale_x_continuous("MAE (minutes)")
p3 <- ggplot(d, aes(mape, m)) +
  geom_point() + geom_segment(aes(xend = 0, yend = m)) +
  scale_x_continuous("MAPE (%)")
p4 <- ggplot(d, aes(pcatch, m)) +
  geom_point() + geom_segment(aes(xend = 0, yend = m)) +
  scale_x_continuous("P(bus arrives after ETA)")
p5 <- ggplot(d, aes(ewait, m)) +
  geom_point() + geom_segment(aes(xend = 0, yend = m)) +
