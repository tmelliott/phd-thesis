...
Accurately predicting the arrival time of a transit vehicle is a decidedly difficult problem. There are some known sources of variability, such as traffic lights and intermediate bus stops, but also countless others which we cannot model. \Cite{} used intervals to estimate, and had limited success? More recently, \citet{} investigated uncertainty displays, and found good evidence to suggest that commuters were able to make use of this additional information. We will examine the use of a single point estimate in an attempt to balance reliability and accuracy and compare this with the use of an interval estimate to convey uncertainty.
