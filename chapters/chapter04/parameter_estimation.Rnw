...
<<tt_future_day,echo=FALSE,fig.height=9,fig.width=8,fig.align="center",out.width="\\linewidth",fig.cap="Travel times along a road one week in the future.">>=
library(ggplot2)
source("load_nw_data.R")
...
do.call(egg::ggarrange, c(
    do.call(c, lapply(seq_along(sids), function(l) {
        sid <- sort(sids)[l]
