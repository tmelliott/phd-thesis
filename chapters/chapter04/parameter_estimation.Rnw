...
if (!file.exists(nwsfile)) {
    ...
    model{
        ...
        for (l in 1:L) {
            ...
            for (j in c1[l]:cJ[l]) {
                beta[j] ~ dnorm(beta[j-1], pow(delta[j] * q[l], -2))T(0, mu[l])
            }

            phi[l] <- exp(log_phi[l])
            log_phi[l] ~ dnorm(theta, pow(sig_phi, -2))

            #q[l] ~ dnorm(mu_q, pow(sig_q, -2))T(0,)
            q[l] ~ dgamma(0.001, 0.001)
        }
...
egg::ggarrange(
    pmin +
        geom_path(aes(y = theta),
            data = n1_samples %>% spread_draws(theta)
        ) +
...
vn <- c("theta", "sig_phi")#, "mu_q", "sig_q")
restbl <-
    tibble(
        Parameter = c("\\theta", "\\sigma_\\psi"),
                      #"\\mu_q", "\\sigma_q"),
