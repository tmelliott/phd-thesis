...
\NWstate_{\ellc} &\sim \Normal{\NWstate_{\ell-1,c}}{(\NWtdiff_c \NWnoise_{\ell})^2} &
...
<<nw_model_n1_view,message=FALSE,echo=FALSE,fig.height=5,fig.width=8,out.width="0.8\\linewidth",fig.cap="Model output for the parameters $\\NWnoise$ and $\\NWvar$, with 95\\% posterior credible region denoted by dashed grey lines.",fig.align="center">>=
...
<<nw_model_n1_kf,echo=FALSE,message=FALSE,warning=FALSE,fig.width=6,fig.height=2,fig.align="center",out.width=".8\\textwidth",fig.cap="Fitted KF to observed speed data.",fig.subcap=c('Estimated mean speed, showing predicted (blue) and updated (red) state estimates.', 'Predictive distribution of vehicle speeds, which includes between-vehicle variability.'),fig.ncol=1,fig.sep=rep("\\\\", 4)>>=
source("scripts/fit_nw_jags.R")
library(ggplot2)
N <- nrow(tts)
M <- length(unique(t30))
...
tt_all %>% group_by(segment_id) %>%
    ...
    filter(speed > 0.5 & speed < 100 / 3.6) %>%
    summarize(sp = mean(speed), sd = sd(speed)) %>%
    filter(sd > 0) %>%
    # mutate(our6 = as.factor(segment_id %in% sids)) %>%
    # arrange(our6) %>%
    ggplot() +
        geom_point(aes(sp, sd), na.rm = TRUE) +
        # geom_smooth(aes(tt, sd, colour = our6), method="lm") +
        #scale_x_log10() +
        #scale_y_log10() +
        xlab("Mean speed (m/s)") +
        ylab("Speed variance (m/s/s)") +
        theme_classic() +
        annotate("text", 13, 4, label = "DELETE ME", size = 10, colour = "red")
@
...
\NWstate_{\ellc} &\sim \Normal{\NWstate_{\ellc-1}}{(\NWtdiff_c \NWnoise_{\ell})^2} &
...
<<nw_seg_maps,echo=FALSE,message=FALSE,cache=TRUE,fig.width=8,fig.height=9,out.width="\\textwidth",fig.cap="Segment locations, drawn using the ggmap package \\citep{ggmap}.",fig.align="center">>=
...
<<nw_model_n2_diag_2,cache=TRUE,echo=FALSE,fig.width=6,fig.height=8,out.width=".8\\textwidth",fig.cap="Traceplots of network parameters for each segment.",fig.align="center">>=
source("scripts/load_nw_data.R")
load("data/hier_model_samples.rda")
