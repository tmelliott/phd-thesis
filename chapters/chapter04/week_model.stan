  ...
  for (p1 in 0:1) {
    for (p2 in 0:1) {
      ...
      for (n in 1:N) {
        ...
        lp[p1 + 2 * p2 + 1] += normal_lpdf(b[n] | eta, err);
      }
    }
  }
}
model {
  ...
  for (j in 1:2) {
    for (m in 1:M) {
      tau[j,m] ~ normal(mu_tau[j], sigma_tau[j]);
      rho[j,m] ~ normal(mu_rho[j], sigma_rho[j]);
    }
  }
