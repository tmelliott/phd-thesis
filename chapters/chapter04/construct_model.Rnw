...
<<nw_sim_data,echo=FALSE,fig.width=8*.8,fig.height=2,fig.align="center",out.width="0.8\\textwidth",fig.cap="Simulated data of average vehicle speed along a road segment. The right hand axis is shown as a guide for readers not familiar with speeds measured in meters per second.",fig.subcap=c('Underlying average speed along segment', 'Average speeds of individual buses', 'Observed average speeds of buses, with vertial lines to denote measurement error.'),fig.ncol=1,fig.sep=rep("\\\\", 4)>>=
...
<<tt_figure,message=FALSE,warning=FALSE,echo=FALSE,fig.height=4,fig.width=8*.8,fig.align="center",out.width="0.8\\linewidth",fig.cap="Road state observations along a single road segment over time, comparing travel time (top) to speed (bottom).",fig.scap="Road state observations along a single road segment.">>=
library(ggplot2)
library(patchwork)
