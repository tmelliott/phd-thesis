...
The relationship between the underlying road speed and the value we estimate (back in \cref{cha:vehicle_model}) involves two steps. First, we must describe the relationship between the average traffic speed and the vehicle's true average speed. Second, we define the relationship between \emph{actual} and \emph{observed} (or, more specifically, estimated) average speed. This type of model is referred to as a \emph{heirarchical (Bayesian) model} and is demonstrated graphically in \cref{fig:nw_model_hierarchy}.
...
\end{equation}
a vector containing the current real-time average traffic speed along all $L$ roads in the network. However, modelling the full state would increase computational demand as $L$ gets large, so, for now, we only consider each road segment $\ell$ \emph{independently} (see later in \cref{sec:} for a discussion of the implications of this).
...
\NWstate_{\ellc} \sim \TNormal{\NWstate_{\ellc-1}}{(\NWtdiff_c \NWnoise_c)^2}{0}{\MaxSpeed_\ell}
\end{equation}
where $\NWtdiff_c = t_c - t_{c-1}$ is the time since the last update. This model allows traffic speed to follow temporal trends (peak hour traffic, for example) as well as react to real-time events, such as accidents or weather events.


As already mentioned, there are many factors which can affect the speed of a vehicle along a given road segment. To encapsulate this uncertainty, we introduce a single parameter $\NWvar_{\ellc}$, which describes the width of the distribution at the top of \cref{fig:nw_model_hierarchy}, or the \emph{between-vehicle varibility}. The true average speed of a bus $m$ along road segment $\ell$ at time $t_c$ is denoted $\Vtt_{\ellc}^m$, and is assumed to be an observation from the population distribution with mean $\NWstate_{\ellc}$ and variance $\NWvar_{\ellc}^2$,
\begin{equation}\label{eq:nw_vehicle_tt}
\Vtt_{\ellc}^m \sim \TNormal{\NWstate_{\ellc}}{\NWvar_{\ellc}^2}{0}{\MaxSpeed_\ell}.
\end{equation}
...
<<tt_figure,message=FALSE,warning=FALSE,echo=FALSE,fig.height=2,fig.width=8,fig.align="center",out.width="\\linewidth",fig.cap="Road state observations along a single road segment over time, with marginal historgrams on the right.",fig.subcap=c("Travel time observations in seconds.", "Average speed in meters per second."),fig.ncol=1,fig.sep=rep("\\\\", 3)>>=
library(ggplot2)
library(patchwork)
source("scripts/load_nw_data.R")

ptt <- ggplot(tts, aes(arrival_time, travel_time)) +
    geom_point() +
    theme_classic() +
    xlab("Time") + ylab("Travel time (seconds)")

htt <- ggplot(tts, aes(travel_time)) +
...
ps <- ggplot(tts, aes(arrival_time, length / travel_time)) +
    geom_point() +
    theme_classic() +
...
hs <- ggplot(tts, aes(length / travel_time)) +
    geom_histogram(bins = 30) +
    coord_flip() +
    theme_classic() +
    theme(
