...
seg_fits <-
    lapply(sids, function(sid) {
        week0_smry <- data_week0 %>%
            ...
            do((.) %>% mutate(
                tt10 = c(mean[-(1:2)], rep(NA, 2)),
                tt20 = c(mean[-(1:4)], rep(NA, 4)),
                tt30 = c(mean[-(1:6)], rep(NA, 6))
            )) %>%
            ungroup()
...
seg_preds <-
    lapply(sids, function(sid) {
        week1_smry <- data_week1 %>%
            ...
            ungroup() %>%
            mutate(segment_id = sid) %>% filter(weekday == "weekday") %>%
            select(segment_id, date, t_int, mean, e, tt30)
    ...
    left_join(seg_fits, by = c("segment_id", "t_int", "mean"))
