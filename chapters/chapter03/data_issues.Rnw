...
<<bus_going_backwards,echo=FALSE,fig.width=6,fig.height=1,out.width=".8\\textwidth",fig.align="center",fig.cap="Three sequential observations of a vehicle approaching an intersection (hollow circle). When the bus nears the intersection, it reports its position exactly at the intersection ($\\Vobs_2$); however, there is a queue at this intersection, so the next observation ($\\Vobs_3$) is behind the previous one, demonstrating the ``reversing bus'' phenomenon.",fig.scap="Visual demonstration of the ``reversing bus'' phenomenon">>=
