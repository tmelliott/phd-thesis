...
<<transition_demo,echo=FALSE,fig.width=8*.33,fig.height=3,out.width=".33\\textwidth",fig.cap="Simulated particle trajectories using three transition functions. Points have been coloured by their parent (after resampling) to demonstrate the affect of system noise.",fig.align="center",fig.subcap=paste0("$\\Vtrans_{A", 1:3, "}$")>>=
