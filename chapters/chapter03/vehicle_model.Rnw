...
or \emph{bus stops},
which are therefore an integral component of our model.
...
The main difficulty here is related to the way in which the data are observed,
which was discussed in \cref{sec:vp_data}.
It is quite common for \GPS{} to only be observed at bus stops,
a location at which there is positive probability of landing.
Thus, it makes sense that the maximum likelihood estimate of the
trajectory between two bus stops puts the maximum possible time
\emph{at the stop}, resulting in maximum speed between stops.
Early iterations of our model succummed to this problem
until we implemented a somewhat strict likelihood function
on the trip updates (see \cref{sec:lhood_trip}).


The typical behaviour at bus stops is that a bus will stop
if there are passengers signalling to get on or off the bus.
Probabilistically, we say that a bus servicing route $r$
will stop at bus stop $m \in \{1, \ldots, \Nstop_r\}$
with some unknown probability $\Prstop_m$.
In those situations when the bus does not stop,
the behaviour follows model A.
...
Step 2 is modelled as the service time of the stop,
...
So, for a given vehicle approaching stop $m$,
the dwell time associated with that stop
can be expressed as a function of
\begin{equation}
\Istop_m \sim \Bern{\Prstop_m}\quad\text{and}\quad
\pserve_m \sim \TNormal{\dwell_m}{\dwellvar_m}{\gamma}{\infty}
\end{equation}
