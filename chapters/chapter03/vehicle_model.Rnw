...
<<sim1_pf_full,echo=FALSE,message=FALSE,fig.width=8,fig.height=3,fig.cap="RMSE results for 100 simulations of each model and sampling method.">>=
...
s00 <- lapply(1:100, function(seed) {
    ...
    set.seed(seed)
    while(is.null(x))
        x <- doSim(n, seg1, seg2, Nparticle, simnames, seed = sample(1e9, 1), fn = seed)
    x
}) %>% bind_rows()

s00 %>% filter(segment == "Segment 1") %>%
    group_by(model, sim) %>% summarize(rmse = median(rmse)) %>%
