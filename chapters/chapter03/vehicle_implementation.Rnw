...
\textbf{This should only be about the specifics of the particle filter's C++ implementation,
not the program itself (which should be introduced in Ch2).}

The implementation of the particle filter in a way that is both
generalised---%
