...
The likelihood function is where the \pf{} shines in this application. The real advantage is in the flexibility of the proposal distribution, in that it is not restricted to conform to a fixed number of modes, for example when passing $m$ stops in a time interval, there are (assuming constant speed) $2^m$ modes in the proposal distribution, two for each stop. Also, not all of the modes have the same width: some are narrow (when the bus doesn't stop), while others are wide (when it does stop with unknown dwell time).


The \pf{} effectively checks to see how plausible the observation is assuming each particle is the truth, allowing us to weight each particle by its plausibility. Since there are two types of data, two likelihood functions are required: one for \GPS{} observations, and a second for trip updates.
