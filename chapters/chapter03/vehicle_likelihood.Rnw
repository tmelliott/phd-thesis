...
d <- tibble(
    ...
    Td = Ta + w,
    stopped = ifelse(w > 0, "stopped", "did not stop"),
    t2 = t + 30L,
    d2 = Sd + (t2 - Td) * v,
    lh = dnorm(Ta, A, 2),
    wt = lh / sum(lh)
...
ggplot(d, aes(alpha = wt, colour = stopped)) +
