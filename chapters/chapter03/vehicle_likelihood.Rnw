...
<<gps_projection,echo=FALSE,message=FALSE,cache=TRUE,fig.width=4,fig.height=3.5,out.width="0.49\\textwidth",fig.align="center",fig.cap="Equirectangular projection of GPS coordinates ({\\sc a}) onto a flat surface ({\\sc b}), which allows the easy calculation of distance between a particle (black) and the vehicle's observed location (orange).",fig.scap="Equirectangular projection of GPS coordinates onto a flat surface",fig.subcap=c("GPS coordinates","Projected points")>>=
