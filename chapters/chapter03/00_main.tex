...
{\centering \subfloat[Observations (points) with possible trajectories which fit the data. The horizontal dashed line represents the location of the stop, and the vertical dotted line indicates the time point at which we wish to estimate vehicle speed (below).\label{fig:vobs_multimode1}]{\includegraphics[width=\textwidth]{figure/vobs_multimode-1} }\newline
\subfloat[Estimated speed distribution at the indicated time (dotted vertical line top graph).\label{fig:vobs_multimode2}]{\includegraphics[width=\textwidth]{figure/vobs_multimode-2} }

}

\caption[An example of multimodality in vehicle state]{An example of multimodality in vehicle state. Note that the effects have been exaggerated here to illustrate the issue.}\label{fig:vobs_multimode}
\end{figure}
