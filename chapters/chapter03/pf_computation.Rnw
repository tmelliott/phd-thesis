...
Since the vehicles are modelled independently, and they only need to read from the GTFS object (not modify it) the vehicle update step is easily parallelised to use $\tilde M$~cores using the OMP library (citation?). This enables up to a $\tilde M$-fold increase in the speed of this step.
