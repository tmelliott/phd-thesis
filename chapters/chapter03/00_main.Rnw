...
In this chapter, we develop a model of the observed data $Y$ for each vehicle,
using the GTFS network developed in section~\ref{sec:route-segments}.
We have chosen to implement it in real-time using a particle filter,
...
\Sexpr{knit_child('realtime_data.Rnw')}
\Sexpr{knit_child('vehicle_model.Rnw')}
\Sexpr{knit_child('vehicle_likelihood.Rnw')}
\Sexpr{knit_child('particle_filter.Rnw')}
