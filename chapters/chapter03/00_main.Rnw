...
<<vobs_multimode,echo=FALSE,fig.width=8,fig.height=2,out.width="\\textwidth",fig.cap="An example of multimodality in vehicle state. Note that the effects have been exaggerated here to illustrate the issue.",fig.align="center",fig.subcap=c("Observations (points) with possible trajectories which fit the data. The horizontal dashed line represents the location of the stop, and the vertical dotted line indicates the time point at which we wish to estimate vehicle speed (below).","Estimated speed distribution at the indicated time (dotted vertical line top graph)."),fig.ncol=1>>=
suppressPackageStartupMessages(library(tidyverse))
library(patchwork)
vd <- tibble(
    id = paste("Vehicle", c("A", "A", "A", "A", "B", "B", "B", "C", "C", "C")),
...
vdt <- vdt %>% as_tibble %>%
    mutate(
        id = factor(id, labels = c("Vehicle A", "Vehicle B", "Vehicle C")),
        id = as.character(id)
    ) %>%
    group_by(id, stop) %>%
    group_modify(~ {
...
p2 <- bind_rows(
    vdt %>% filter(id == "Vehicle A" & t == 70),
    vdt %>% filter(id == "Vehicle B" & t == 20),
    vdt %>% filter(id == "Vehicle C" & t == 60)
) %>%
    select(id, speed) %>%
    group_by(id) %>%
    group_modify(~ {
