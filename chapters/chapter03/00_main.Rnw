...
We have chosen to implement it in \rt{} using a \pf{},
which allows much more flexibility in the model,
which is detailed in \cref{sec:vehicle_model}.
Following this, travel time estimation is implemented using the results
from the \pf{} model,
and the models compared using a simulation in \cref{sec:vehicle_travel_times}.
The design and implementation of the \pf{} is discussed later in
