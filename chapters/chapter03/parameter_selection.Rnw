...
(there's the trip updates dwell times, but they have that weird 9~s issue).

\textcolor{red}{
    \begin{itemize}
        \item create a new AT developer account
        \item collect trip updates every 5 seconds for a few days to try and catch as many arrival/departure times as possible
        \item then look at that distribution
    \end{itemize}
}
...
<<observed_dwell,message=FALSE,warning=FALSE,echo=FALSE,fig.width=8,fig.height=3,fig.cap="Distribution of dwell times observed over the course of five days.">>=
library(tidyverse)
...
ggplot(dwell %>% filter(dwell < 2*60)) +
    geom_histogram(aes(dwell, stat(density)), bins = 120) +
...
To these data we could fit some model ... and find approximate values
of dwell time over all/by route/by trip/time of day.
% \begin{equation}
...
\textcolor{red}{
    \begin{itemize}
        \item in particle filter they're not really useful because
            even if we know the mean, it's going to be highly variable
        \item somewhat useful for uncertainty calculations of ETAs
    \end{itemize}
}
