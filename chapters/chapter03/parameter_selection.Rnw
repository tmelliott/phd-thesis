...
<<observed_dwell,cache=TRUE,message=FALSE,warning=FALSE,echo=FALSE,fig.width=8,fig.height=3,fig.cap="Distribution of dwell times observed over the course of five days, truncated at one minute.">>=
library(tidyverse)
...
ggplot(dwell %>% filter(dwell < 60)) +
    ...
    geom_path(aes(x, p),
        data = tibble(
            ...
            p = truncnorm::dtruncnorm(x - gamma, 0, Inf, 18, 10)
        ),
        lty = 2, colour = "red"
    )
@
