...
tt <- s1$segments$tt
fcap1 <- 'Simulated vehicle trajectory, with five full road segments, along with simulated observations with three sampling techniques.'
fscap1 <- c(
...
<<sim1_graph,echo=FALSE,fig.height=4,fig.width=8,out.width="\\linewidth",fig.cap=fcap1,warning=FALSE,message=FALSE>>=
...
<<sim1_pf_full,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=2.5,out.width="\\linewidth",fig.cap="Speed estimation results for 100 simulations. In each the vehicle trajectory is simulated using a different seed, and the speed estimate is taken as the mean of the particle speeds along each segment.">>=
...
<<sim2_pf,echo=FALSE,message=FALSE,warning=FALSE,fig.height=2.5,fig.width=8,out.width="\\linewidth",fig.cap="Simulation results for the three models (rows) applied to the data from three sampling methods (columns), using $\\Np=2000$ particles. [details].">>=
S0 <- sim2$fits
S0 %>%
    filter(segment != "Segment 6") %>%
    mutate(
...
<<sim2_pf_full,echo=FALSE,message=FALSE,warning=FALSE,fig.width=8,fig.height=3,fig.cap="Speed estimation results for 100 simulations. In each the vehicle trajectory is simulated using a different seed, and the speed estimate is taken as the mean of the particle speeds along each segment.">>=
library(parallel)
cl <- makeCluster(4L)
r <- clusterEvalQ(cl, source("scripts/pf_simulation.R"))
