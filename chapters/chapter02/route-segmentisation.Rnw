...
<<gtfs_route_network,echo=FALSE,message=FALSE,fig.width=8*.6,fig.height=4,out.width="0.6\\textwidth",fig.align="center",fig.cap="Simplified network diagram with road segments constructed from stop sequences. Top: routes are coloured individually. Bottom: road segments are sized by the number of associated routes.">>=
library(tidyverse)
