...
<<tu_eta_jump,echo=FALSE,fig.width=6,fig.height=3,out.width=".8\\textwidth",fig.align="center",fig.cap="The current system displays ETAs to passengers as integer minutes which decrease over time. When new data is recieved (marked by an arrow) the ETA is updated, which may result in a sudden ``jump'', as demonstrated.",fig.scap="Demonstration of how ETAs are percieved by passengers under the current system",message=FALSE,warning=FALSE>>=
