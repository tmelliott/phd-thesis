...
if (outdated(url)) {
    ...
    for (file in files) {
        ...
        dbWriteTable(con, tbl, read.csv(file, header = TRUE))
    }
    dbDisconnect(con)
    unlink(dir)
}
...
con <- dbConnect(SQLite(), db)
nroute <- dbGetQuery(con, "select count(*) as n from routes")$n
ntrip <- dbGetQuery(con, "select count(*) as n from trips")$n
ntrip.week <- dbGetQuery(con, "select count(*) as n from calendar where friday=1")$n
dbDisconnect(con)
