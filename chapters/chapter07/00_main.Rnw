...
The other difficulty with \glspl{rbm} for real-time vehicle location data is the prediction step, particularly when passing a stop or through a \emph{loop}. In such situations, the bus could potentially be in two distinct \emph{states} (different distances travelled). When passing stops, only the state prediction is affected and the underlying event can be distinguished having observed the data.
