...
To find the coordinates of a point 40~meters along this line, we find the maximum point along the route with \emph{shape distance travelled} less than 40, which is the point in red in \cref{fig:app:measure_step1}, which is \Sexpr{wd}~meters along the route. There remains $40-\Sexpr{wd}=\Sexpr{40 - wd}$~meters to travel to get to 40~meters.

<<app:measure_step1,echo=FALSE,fig.width=8*.6,fig.height=2.5,out.width=".6\\textwidth",fig.align="center",fig.cap="The farthest point along the route less than 40~meters along the route (red).",fig.pos="ht">>=
